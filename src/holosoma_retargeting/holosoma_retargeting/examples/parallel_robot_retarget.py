...
def generate_augmentation_configs(
    ...
    if task_type == "object_interaction":
        ...
        if augmentation:
            mode = task_config.object_augmentation_mode if task_config is not None else "all"
            ...
            if mode in {"all", "scale"}:
                scale_variants = task_config.object_scale_variants if task_config is not None else ()
                for scale_values in scale_variants:
                    scale = np.asarray(scale_values, dtype=float)
                    scale_name = "_".join(f"{value:.2f}" for value in scale)
                    augmentations.append(
                        {
                            "name": f"scale_{scale_name}",
            ...
            if mode in {"all", "mesh"}:
                mesh_variants = task_config.object_mesh_variants if task_config is not None else ()
                for index, mesh_path in enumerate(mesh_variants):
                    augmentations.append(
                        {
                            "name": f"mesh_{index}_{Path(mesh_path).stem}",
                            "translation": np.zeros(3),
                            "rotation": 0.0,
                            "scale": np.ones(3),
                            "mesh": Path(mesh_path),
...
def process_single_task(args):
    ...
    augmentations = generate_augmentation_configs(task_type, augmentation, task_config)
    ...
    for k, aug_config in enumerate(augmentations):
        ...
        if task_type == "climbing":
            ...
            object_local_pts, object_local_pts_demo, object_urdf_path = setup_object_data(
                ...
                augmentation=(k > 0),
                object_scale_augmented=aug_config["scale"],
                object_mesh_augmented=aug_config["mesh"],
                generated_object_dir=Path(save_dir) / "_generated_objects" / task_name,
