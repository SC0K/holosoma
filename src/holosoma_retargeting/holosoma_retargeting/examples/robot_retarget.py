...
def create_task_constants(
    ...
    if task_type == "robot_only":
        ...
        task_constants.SCENE_XML_FILE = None
...
def setup_object_data(
    ...
    object_mesh_augmented: Path | None = None,
    generated_object_dir: Path | None = None,
    augmentation_name: str = "augmented",
    ...
    Args:
        ...
        object_mesh_augmented: Optional replacement box OBJ for object interaction
        generated_object_dir: Directory for generated URDF and MuJoCo scene files
        augmentation_name: File-safe name for the generated object variant
    Returns:
    ...
    object_scale_augmented = np.asarray(object_scale_augmented, dtype=float)
    if object_scale_augmented.shape != (3,) or np.any(object_scale_augmented <= 0):
        raise ValueError("object_scale_augmented must contain three positive values")
    ...
    if task_type == "object_interaction":
        ...
        object_local_pts_original, object_local_pts_demo = load_object_data(
            constants.OBJECT_MESH_FILE, smpl_scale=smpl_scale, sample_count=100
        )
        object_local_pts = object_local_pts_original
        object_urdf_file = constants.OBJECT_URDF_FILE
        base_scene_xml = constants.ROBOT_URDF_FILE.replace(
            ".urdf", f"_w_{constants.OBJECT_NAME}.xml"
        )
        constants.SCENE_XML_FILE = base_scene_xml

        geometry_changed = augmentation and (
            object_mesh_augmented is not None
            or not np.allclose(object_scale_augmented, object_scale_normal)
        )
        if geometry_changed:
            if generated_object_dir is None:
                generated_object_dir = Path("generated_objects")
            generated_object_dir.mkdir(parents=True, exist_ok=True)

            if object_mesh_augmented is not None:
                object_mesh_augmented = Path(object_mesh_augmented)
                if not object_mesh_augmented.is_file():
                    raise FileNotFoundError(f"Target object mesh not found: {object_mesh_augmented}")
                object_local_pts = map_points_between_box_meshes(
                    object_local_pts_original,
                    constants.OBJECT_MESH_FILE,
                    object_mesh_augmented,
                )
            else:
                object_local_pts = object_local_pts_original * object_scale_augmented

            object_urdf_file = create_object_variant_urdf(
                constants.OBJECT_URDF_FILE,
                generated_object_dir / f"{constants.OBJECT_NAME}_{augmentation_name}.urdf",
                scale=tuple(float(v) for v in object_scale_augmented),
                mesh_path=object_mesh_augmented,
            )
            constants.SCENE_XML_FILE = create_object_variant_scene_xml(
                base_scene_xml,
                f"{constants.OBJECT_NAME}_mesh",
                generated_object_dir / f"{Path(base_scene_xml).stem}_{augmentation_name}.xml",
                scale=tuple(float(v) for v in object_scale_augmented),
                mesh_path=object_mesh_augmented,
            )
...
def main(cfg: RetargetingConfig) -> None:
    ...
    object_local_pts, object_local_pts_demo, object_urdf_path = setup_object_data(
        ...
        cfg.task_config,
        cfg.augmentation,
        object_scale_augmented=np.asarray(cfg.task_config.object_scale),
        object_mesh_augmented=cfg.task_config.augmentation_object_mesh,
        generated_object_dir=save_dir / "_generated_objects" / task_name,
    )
