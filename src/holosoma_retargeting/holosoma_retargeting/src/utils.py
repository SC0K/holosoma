...
def map_points_between_box_meshes(
    points: np.ndarray,
    source_mesh_path: str | Path,
    target_mesh_path: str | Path,
) -> np.ndarray:
    """Transfer points between axis-aligned box meshes by normalized coordinates.

    This preserves point correspondence even when the two box meshes use
    different vertex counts or triangulations.
    """
    source_mesh = trimesh.load(source_mesh_path, force="mesh")
    target_mesh = trimesh.load(target_mesh_path, force="mesh")
    source_min, source_max = np.asarray(source_mesh.bounds)
    target_min, target_max = np.asarray(target_mesh.bounds)
    source_extent = source_max - source_min
    target_extent = target_max - target_min
    if np.any(source_extent <= 0) or np.any(target_extent <= 0):
        raise ValueError("Box meshes must have non-zero extent on all three axes")

    normalized = (np.asarray(points) - source_min) / source_extent
    return target_min + normalized * target_extent


def create_object_variant_urdf(
    urdf_path: str | Path,
    output_path: str | Path,
    scale: tuple[float, float, float] = (1.0, 1.0, 1.0),
    mesh_path: str | Path | None = None,
) -> str:
    """Create an object URDF using a scaled source mesh or a replacement mesh."""
    urdf_path = Path(urdf_path).resolve()
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)

    tree = ET.parse(urdf_path)
    meshes = tree.getroot().findall(".//mesh")
    if not meshes:
        raise ValueError(f"No mesh elements found in object URDF: {urdf_path}")

    scale_text = " ".join(f"{value:g}" for value in scale)
    for mesh in meshes:
        if mesh_path is None:
            source = (urdf_path.parent / mesh.attrib["filename"]).resolve()
            mesh.set("filename", str(source))
            mesh.set("scale", scale_text)
        else:
            mesh.set("filename", str(Path(mesh_path).resolve()))
...
def create_object_variant_scene_xml(
    ...
    """Create a robot-object MuJoCo scene with replacement object geometry."""
    ...
    if compiler is not None and compiler.get("meshdir"):
        meshdir = (scene_xml_path.parent / compiler.get("meshdir", "")).resolve()
        compiler.set("meshdir", str(meshdir))

    object_mesh = root.find(f"./asset/mesh[@name='{object_mesh_name}']")
    if object_mesh is None:
        raise ValueError(f"Mesh asset '{object_mesh_name}' not found in MuJoCo scene: {scene_xml_path}")

    if mesh_path is None:
        object_mesh.set("scale", " ".join(f"{value:g}" for value in scale))
    else:
        object_mesh.set("file", str(Path(mesh_path).resolve()))
        object_mesh.set("scale", "1 1 1")
