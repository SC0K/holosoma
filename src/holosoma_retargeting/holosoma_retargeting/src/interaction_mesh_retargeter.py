...
class InteractionMeshRetargeter:
    ...
    def retarget_motion(
        ...
        Args:
            human_joint_motions (np.ndarray | None): (num_frames, num_joints, 3) array.
                Set to None when source_robot_motions provides a robot-space source.
            ...
            source_robot_motions (np.ndarray, optional): Source robot qpos used to
                build a robot-to-robot interaction mesh instead of human joints.
            fps (float): Frame rate stored in the output file.
            output_metadata (dict, optional): Extra scalar metadata stored in the output file.

        Returns:
            tuple: (retargeted_motions, obj_pts_demo_list, obj_pts_list, tetrahedra)
        """
        if (human_joint_motions is None) == (source_robot_motions is None):
            raise ValueError("Provide exactly one of human_joint_motions or source_robot_motions")
        source_motions = human_joint_motions if source_robot_motions is None else source_robot_motions
        num_frames = source_motions.shape[0]
        if num_frames == 0:
            raise ValueError("Source motion is empty")
        if source_robot_motions is not None:
            source_robot_motions = np.asarray(source_robot_motions)
            if source_robot_motions.shape != (num_frames, self.nq):
                raise ValueError(
                    f"source_robot_motions must have shape ({num_frames}, {self.nq}), "
                    f"got {source_robot_motions.shape}"
                )
        ...
        if q_nominal_list is not None:
            q_locked_list = np.asarray(q_nominal_list).copy()
        else:
        ...
        with tqdm(range(num_frames)) as pbar:
            for i in pbar:
                ...
                # Build source robot/object relationships in the object frame.
                if source_robot_motions is None:
                    source_mapped_joints = human_joint_motions[i, self.smplh_mapped_joint_indices]
                else:
                    source_mapped_joints = self._get_robot_link_positions(
                        source_robot_motions[i], self.laplacian_match_links.values()
                    )

                if self.object_name == "ground":
                    source_mapped_joints_in_object = source_mapped_joints
                else:
                    source_mapped_joints_in_object = transform_points_world_to_local(
                        object_quat_demo, object_trans_demo, source_mapped_joints
                    )
                ...
                source_vertices, source_tetrahedra = create_interaction_mesh(
                    np.vstack([source_mapped_joints_in_object, obj_pts_demo_i])
                )
                ...
                if self.debug:
                    ...
                    human_kpts_handle_list = self.draw_keypoints(
                        source_mapped_joints, name="source_kpts"
                    )  # 15 X 3
        ...
        if source_robot_motions is not None:
            save_data["source_qpos"] = source_robot_motions
        if output_metadata:
            save_data.update(output_metadata)
        Path(dest_res_path).parent.mkdir(parents=True, exist_ok=True)
        np.savez(dest_res_path, **save_data)
    ...
    def _calc_contact_jacobian_from_point(self, body_idx: int, p_body: np.ndarray, input_world=False):
        ...
        body_id = int(np.asarray(body_idx).reshape(-1)[0])
        ...
        mujoco.mj_jac(self.robot_model, self.robot_data, Jp, Jr, p_W, body_id)  # Jp = J_v
    ...
    def extract_robot_foot_sticking_sequence(self, qpos, velocity_threshold=0.01):
        """Detect left/right foot sticking directly from a robot qpos trajectory."""
        qpos = np.asarray(qpos)
        left_links = [name for name in self.foot_links if "left" in name.lower()]
        right_links = [name for name in self.foot_links if "right" in name.lower()]
        if not left_links or not right_links:
            raise ValueError("Robot foot links must include both left and right links")

        left_positions = np.array(
            [self._get_robot_link_positions(q, left_links).mean(axis=0) for q in qpos]
        )
        right_positions = np.array(
            [self._get_robot_link_positions(q, right_links).mean(axis=0) for q in qpos]
        )
        left_velocity = np.linalg.norm(np.diff(left_positions[:, :2], axis=0), axis=1)
        right_velocity = np.linalg.norm(np.diff(right_positions[:, :2], axis=0), axis=1)
        ...
        return [
            {
                "L_Toe": bool(left_velocity[i] <= velocity_threshold),
                "R_Toe": bool(right_velocity[i] <= velocity_threshold),
            }
