...
from holosoma_retargeting.src.qpos_layout import convert_qpos_layout, detect_qpos_layout  # noqa: E402


def load_npz(npz_path: str, input_layout: str = "auto"):
    ...
    if resolved_layout == "auto":
        resolved_layout = detect_qpos_layout(qpos)
    if resolved_layout not in {"native", "omniretarget"}:
        raise ValueError(f"Unsupported qpos layout: {resolved_layout!r}")
    qpos = convert_qpos_layout(qpos, resolved_layout, "native")
    print(f"[viser_player] qpos layout: {resolved_layout} -> native")
    return qpos, fps
...
def main(cfg: ViserConfig) -> None:
    ...
    qpos, fps = load_npz(cfg.qpos_npz, cfg.input_layout)
    make_player(
        config=cfg,
        qpos=qpos,
        fps=fps,
